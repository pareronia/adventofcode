...
def _parse(inputs: tuple[str]) -> tuple[Sensors, Beacons]:
    ...
    for line in inputs:
        ...
        sensors.append((sx, sy, _distance(sx, sy, bx, by)))
        beacons[by].add(bx)
    return sensors, beacons


def _distance(x1, y1, x2, y2) -> int:
    return abs(x1 - x2) + abs(y1 - y2)


def _get_ranges(sensors: Sensors, beacons: Beacons, y: int) -> list[Range]:
    ranges = set[Range]()
    for sx, sy, d in sensors:
        dy = abs(sy - y)
        if dy > d:
            continue
        ranges.add((sx - d + dy, sx + d - dy))
    return _merge_ranges(ranges)


def _merge_ranges(ranges: set[Range]) -> list[Range]:
    m = list[Range]()
    for s_min, s_max in sorted(r for r in ranges):
        if len(m) == 0:
            m.append((s_min, s_max))
            continue
        last_min, last_max = m[-1]
        if s_min <= last_max:
            m[-1] = (last_min, max(last_max, s_max))
            continue
        m.append((s_min, s_max))
    return m
...
def solve_1(inputs: tuple[str], y: int) -> int:
    ...
    ans = 0
    for r_min, r_max in _get_ranges(sensors, beacons, y):
        ans += (
            r_max
            - r_min
            + 1
...
def solve_2(inputs: tuple[str], the_max: int) -> int:
    sensors, beacons = _parse(inputs)
    for y in range(the_max + 1):
        ranges = _get_ranges(sensors, beacons, y)
        x = 0
        while x <= the_max:
...
def main() -> None:
    aocd.get_data(year=2022, day=15, block=True)
