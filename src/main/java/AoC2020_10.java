...
public final class AoC2020_10 extends SolutionBase<List<Integer>, Long, Long> {
    ...
    protected List<Integer> parseInput(final List<String> inputs) {
        ...
        inputs.stream().map(Integer::valueOf).sorted().forEach(numbers::add);
    ...
    public Long solvePart2(final List<Integer> numbers) {
        ...
        map.put(0, 1L);
        enumerate(numbers.stream().skip(1)).stream()
                .forEach(
                        e -> {
                            range(e.index(), -1, -1)
                                    .intStream()
        ...
        return map.get(numbers.getLast());
    }
